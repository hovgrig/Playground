...
void XmlRpcServerConnection::generateResponseImpl(std::string const& resultXml, std::string& response)
{
...
{
    generateResponseImpl(resultXml, _response);
}
...
void XmlRpcServerConnection::generateFaultResponseImpl(std::string const& errorMsg, std::string& response, int errorCode)
{
  const char RESPONSE_1[] = 
    "<?xml version=\"1.0\"?>\r\n"
...
{
    generateFaultResponseImpl(errorMsg, _response, errorCode);
}